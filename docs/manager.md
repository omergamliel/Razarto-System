...
| קבלת פופאפ התראה על פעולה של משתמש אחר שרלוונטית לי (בקשת ראש-בראש נכנסת, הצעת/ביטול כיסוי על משמרת שלי, בקשה שלי/שכיסיתי שנסגרה) | `useNotificationScanner.js` (`src/hooks/`) + `notificationEvents.js`/`NotificationSidebar.jsx` (`src/components/sidebar/`) — סריקה של המצב הנוכחי בטעינה/רענון, לא Push חי; ראו `data_flow.md` §7א ו-`components_sidebar.md` |
...
- **אין דרך לדעת מי ביטל בקשת Head2Head — הבעלים או הצד השני שדחה אותה.** `cancelSwapMutation`/`cancelSwapRequestMutation` הוא אותו נתיב קוד גם ל"ביטלתי את הבקשה שלי" וגם ל"דחיתי בקשת Head2Head שנכנסה אליי" (כפתור הדחייה ב-`KPIListModal.jsx` קורא לאותו `onCancelRequest`), ואין שדה כמו `cancelled_by_user_id` שמתעד את המבצע. בגלל זה `notificationEvents.js` (ראו `components_sidebar.md`) מדלג בכוונה על אירוע "הבקשה שלך נדחתה" — היה עלול לייצר פופאפ שגוי כשמדובר בביטול-עצמי.
